...
def test_make_swagger_generated_classes_picklable__api_workflow_client(
    ...
    mock_is_compatible_version = mocker.patch.object(api_workflow_client, "is_compatible_version")
    ...
    assert all(type(client.__dict__[key] == new_client.__dict__[key]) for key in client.__dict__.keys())
...
def test_make_swagger_generated_classes_picklable__api_client() -> None:
    ...
    assert all(original_dict[key] == new_client.__dict__[key] for key in original_dict.keys())
...
def test_make_swagger_generated_classes_picklable__configuration() -> None:
    ...
    assert all(type(config.__dict__[key] == new_config.__dict__[key]) for key in config.__dict__.keys())
    ...
    assert all(original_dict[key] == new_config.__dict__[key] for key in original_dict.keys())
