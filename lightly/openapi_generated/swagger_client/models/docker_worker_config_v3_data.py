...
class DockerWorkerConfigV3Data(BaseModel):
    ...
    def to_dict(self, by_alias: bool = False):
        ...
        if self.config_orig:
            _dict[
                "configOrig" if by_alias else "config_orig"
            ] = self.config_orig.to_dict(by_alias=by_alias)
        return _dict
    ...
    def from_dict(cls, obj: dict) -> DockerWorkerConfigV3Data:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError(
                    "Error due to additional fields (not defined in DockerWorkerConfigV3Data) in the input: "
                    + str(obj)
                )
        ...
        _obj = DockerWorkerConfigV3Data.parse_obj(
            {
                ...
                "config": DockerWorkerConfigV3.from_dict(obj.get("config"))
                if obj.get("config") is not None
                else None,
                "config_orig": DockerWorkerConfigV3.from_dict(obj.get("configOrig"))
                if obj.get("configOrig") is not None
                else None,
