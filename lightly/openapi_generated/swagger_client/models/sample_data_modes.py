...
class SampleDataModes(BaseModel):
    ...
    last_modified_at: Optional[conint(strict=True, ge=0)] = Field(
        None, alias="lastModifiedAt", description="unix timestamp in milliseconds"
    )
    ...
    def to_dict(self, by_alias: bool = False):
        ...
        if self.video_frame_data:
            _dict[
                "videoFrameData" if by_alias else "video_frame_data"
            ] = self.video_frame_data.to_dict(by_alias=by_alias)
        # override the default output from pydantic by calling `to_dict()` of crop_data
        if self.crop_data:
            _dict["cropData" if by_alias else "crop_data"] = self.crop_data.to_dict(
                by_alias=by_alias
            )
    ...
    def from_dict(cls, obj: dict) -> SampleDataModes:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError(
                    "Error due to additional fields (not defined in SampleDataModes) in the input: "
                    + str(obj)
                )
        ...
        _obj = SampleDataModes.parse_obj(
            {
                ...
                "video_frame_data": VideoFrameData.from_dict(obj.get("videoFrameData"))
                if obj.get("videoFrameData") is not None
                else None,
                "crop_data": CropData.from_dict(obj.get("cropData"))
                if obj.get("cropData") is not None
                else None,
            }
        )
        return _obj
