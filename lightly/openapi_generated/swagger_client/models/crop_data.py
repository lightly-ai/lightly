...
class CropData(BaseModel):
    ...
    parent_id: constr(strict=True) = Field(
        ..., alias="parentId", description="MongoDB ObjectId"
    )
    ...
    prediction_index: conint(strict=True, ge=0) = Field(
        ...,
        alias="predictionIndex",
        description="the index of this crop within all found prediction singletons of a sampleId (the parentId)",
    )
    prediction_task_name: constr(strict=True, min_length=1) = Field(
        ...,
        alias="predictionTaskName",
        description="A name which is safe to have as a file/folder name in a file system",
    )
    prediction_task_category_id: conint(strict=True, ge=0) = Field(
        ...,
        alias="predictionTaskCategoryId",
    ...
    def from_dict(cls, obj: dict) -> CropData:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError(
                    "Error due to additional fields (not defined in CropData) in the input: "
                    + str(obj)
                )
