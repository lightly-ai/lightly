...
class DatasourceConfigS3DelegatedAccessAllOf(BaseModel):
    ...
    s3_external_id: constr(strict=True, min_length=10) = Field(
        ...,
        alias="s3ExternalId",
        description="The external ID specified when creating the role.",
    )
    s3_arn: constr(strict=True, min_length=12) = Field(
        ..., alias="s3ARN", description="The ARN of the role you created"
    )
    ...
    def s3_arn_validate_regular_expression(cls, value):
        ...
        if not re.match(r"^arn:aws:iam::[0-9]{12}:role.+$", value):
            raise ValueError(
                r"must validate the regular expression /^arn:aws:iam::[0-9]{12}:role.+$/"
            )
        return value
    ...
    def from_dict(cls, obj: dict) -> DatasourceConfigS3DelegatedAccessAllOf:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError(
                    "Error due to additional fields (not defined in DatasourceConfigS3DelegatedAccessAllOf) in the input: "
                    + str(obj)
                )
