    ...
    def from_dict(cls, obj: dict) -> SelectionConfigV3Entry:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError(
                    "Error due to additional fields (not defined in SelectionConfigV3Entry) in the input: "
                    + str(obj)
                )

        _obj = SelectionConfigV3Entry.parse_obj(
            {
                "input": SelectionConfigV3EntryInput.from_dict(obj.get("input"))
                if obj.get("input") is not None
                else None,
                "strategy": SelectionConfigV3EntryStrategy.from_dict(
                    obj.get("strategy")
                )
