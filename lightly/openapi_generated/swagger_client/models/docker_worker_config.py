...
class DockerWorkerConfig(BaseModel):
    ...
    docker: Optional[Dict[str, Any]] = Field(
        None,
        description="docker run configurations, keys should match the structure of https://github.com/lightly-ai/lightly-core/blob/develop/onprem-docker/lightly_worker/src/lightly_worker/resources/docker/docker.yaml ",
    )
    lightly: Optional[Dict[str, Any]] = Field(
        None,
        description="lightly configurations which are passed to a docker run, keys should match structure of https://github.com/lightly-ai/lightly/blob/master/lightly/cli/config/config.yaml ",
    )
    ...
    def from_dict(cls, obj: dict) -> DockerWorkerConfig:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError(
                    "Error due to additional fields (not defined in DockerWorkerConfig) in the input: "
                    + str(obj)
                )
