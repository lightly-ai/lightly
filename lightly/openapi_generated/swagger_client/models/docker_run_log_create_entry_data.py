...
class DockerRunLogCreateEntryData(BaseModel):
    """
    DockerRunLogCreateEntryData
    """
    ts: conint(strict=True, ge=0) = Field(..., description="unix timestamp in milliseconds")
    level: DockerRunLogLevel = Field(...)
    group: StrictStr = Field(..., description="The logger name/group of the log entry.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> DockerRunLogCreateEntryData:
        """Create an instance of DockerRunLogCreateEntryData from a JSON string"""
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> DockerRunLogCreateEntryData:
        """Create an instance of DockerRunLogCreateEntryData from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return DockerRunLogCreateEntryData.parse_obj(obj)

        # raise errors for additional fields in the input
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in DockerRunLogCreateEntryData) in the input: " + str(obj))

        _obj = DockerRunLogCreateEntryData.parse_obj({
            "ts": obj.get("ts"),
            "level": obj.get("level"),
            "group": obj.get("group"),
