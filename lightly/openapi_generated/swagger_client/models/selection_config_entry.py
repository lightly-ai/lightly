...
class SelectionConfigEntry(BaseModel):
    ...
    def to_dict(self, by_alias: bool = False):
        ...
        if self.input:
            _dict["input" if by_alias else "input"] = self.input.to_dict(
                by_alias=by_alias
            )
        # override the default output from pydantic by calling `to_dict()` of strategy
        if self.strategy:
            _dict["strategy" if by_alias else "strategy"] = self.strategy.to_dict(
                by_alias=by_alias
            )
        return _dict
    ...
    def from_dict(cls, obj: dict) -> SelectionConfigEntry:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError(
                    "Error due to additional fields (not defined in SelectionConfigEntry) in the input: "
                    + str(obj)
                )

        _obj = SelectionConfigEntry.parse_obj(
            {
                "input": SelectionConfigEntryInput.from_dict(obj.get("input"))
                if obj.get("input") is not None
                else None,
                "strategy": SelectionConfigEntryStrategy.from_dict(obj.get("strategy"))
