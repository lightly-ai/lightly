...
class DockerTaskDescription(BaseModel):
    ...
    existing_selection_column_name: StrictStr = Field(
        ..., alias="existingSelectionColumnName"
    )
    active_learning_scores_column_name: StrictStr = Field(
        ..., alias="activeLearningScoresColumnName"
    )
    ...
    n_data: Union[confloat(ge=0, strict=True), conint(ge=0, strict=True)] = Field(
        ...,
        alias="nData",
        description="the number of samples in the current embeddings file",
    )
    __properties = [
        "embeddingsFilename",
        "embeddingsHash",
    ...
    def to_dict(self, by_alias: bool = False):
        ...
        if self.sampling_config:
            _dict[
                "samplingConfig" if by_alias else "sampling_config"
            ] = self.sampling_config.to_dict(by_alias=by_alias)
        return _dict
    ...
    def from_dict(cls, obj: dict) -> DockerTaskDescription:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError(
                    "Error due to additional fields (not defined in DockerTaskDescription) in the input: "
                    + str(obj)
                )
        ...
        _obj = DockerTaskDescription.parse_obj(
            {
                ...
                "existing_selection_column_name": obj.get(
                    "existingSelectionColumnName"
                ),
                "active_learning_scores_column_name": obj.get(
                    "activeLearningScoresColumnName"
                ),
                "masked_out_column_name": obj.get("maskedOutColumnName"),
                "sampling_config": SamplingConfig.from_dict(obj.get("samplingConfig"))
                if obj.get("samplingConfig") is not None
                else None,
