...
class DatasetData(BaseModel):
    ...
    user_id: StrictStr = Field(
        ..., alias="userId", description="The owner of the dataset"
    )
    ...
    meta_data_configuration_id: Optional[constr(strict=True)] = Field(
        None, alias="metaDataConfigurationId", description="MongoDB ObjectId"
    )
    ...
    datasource_processed_until_timestamp: Optional[conint(strict=True, ge=0)] = Field(
        None,
        alias="datasourceProcessedUntilTimestamp",
        description="unix timestamp in seconds",
    )
    access_role: Optional[constr(strict=True)] = Field(
        None,
        alias="accessRole",
        description="AccessRole bitmask of the one accessing the dataset",
    )
    parent_dataset_id: Optional[constr(strict=True)] = Field(
        None, alias="parentDatasetId", description="MongoDB ObjectId"
    )
    original_dataset_id: Optional[constr(strict=True)] = Field(
        None, alias="originalDatasetId", description="MongoDB ObjectId"
    )
    __properties = [
        "id",
        "name",
    ...
    def from_dict(cls, obj: dict) -> DatasetData:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError(
                    "Error due to additional fields (not defined in DatasetData) in the input: "
                    + str(obj)
                )
