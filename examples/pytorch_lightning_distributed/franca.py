...
class Franca(pl.LightningModule):
    def __init__(
        self,
        ...
        # Losses. The nested prototype counts come from the head. gather_distributed
        # synchronizes the Sinkhorn normalization across processes (sinkhorn mode only).
        self.dino_criterion = FrancaDINOLoss(
            output_dims=student_dino_head.output_dims, gather_distributed=True
        )
        self.ibot_criterion = FrancaIBOTPatchLoss(
            output_dims=student_ibot_head.output_dims, gather_distributed=True
        )
